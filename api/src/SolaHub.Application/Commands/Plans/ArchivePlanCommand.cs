...
{
    public async Task<Result<ReadingPlanDto>> Handle(ArchivePlanCommand request, CancellationToken ct)
    {
        ...
        if (plan.CreatedBy != request.RequestingUserId)
            return Error.Forbidden("Plans.Forbidden", "Only the plan creator can archive this plan.");
