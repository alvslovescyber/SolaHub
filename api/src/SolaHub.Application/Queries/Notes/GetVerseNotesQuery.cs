...
public sealed record GetVerseNotesQuery(string VerseRef, bool SharedOnly) : IQuery<IReadOnlyList<NoteDto>>;
...
{
    public async Task<Result<IReadOnlyList<NoteDto>>> Handle(GetVerseNotesQuery request, CancellationToken ct)
    {
        if (!VerseRef.TryParse(request.VerseRef, out var verseRef))
            return Error.Validation("Notes.InvalidVerseRef", $"'{request.VerseRef}' is not a valid verse reference.");
